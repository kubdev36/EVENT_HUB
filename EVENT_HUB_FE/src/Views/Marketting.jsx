    ...
    return sources.map((comp) => {
      const events = (comp.events || []).filter(
        (ev) =>
          ev.date === dateStr &&
          isEventVisible(ev) &&
          (!allowedTypes || allowedTypes.includes(ev.type)) &&
    ...
    <div className="flex h-full min-h-0 w-full overflow-hidden bg-slate-50 text-slate-800 font-sans">
      <div className="flex-1 overflow-y-auto p-3 sm:p-4 lg:p-6 space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          <div>
            <h1 className="text-lg sm:text-xl font-bold text-slate-900">Marketing</h1>
            <p className="text-[11px] sm:text-xs text-slate-500">Chỉ hiển thị sự kiện của phòng Marketing (Quảng cáo)</p>
          </div>
          ...
          <div className="p-2.5 sm:p-3 space-y-3">
            {displayCompetitors.length > 0 ? (
              ...
              <div className="py-12 text-center text-slate-400 text-xs">
                Không có sự kiện Marketing trong ngày này
              </div>
            )}
          </div>
        </div>
